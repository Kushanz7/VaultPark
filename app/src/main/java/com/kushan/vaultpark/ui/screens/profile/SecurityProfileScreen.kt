...
fun SecurityProfileScreen(
    ...
    var editGateLocation by remember { mutableStateOf("") }
    ...
    LaunchedEffect(uiState.user) {
        if (uiState.user != null) {
            ...
            editGateLocation = uiState.user!!.membershipType // Using as gate location
        }
    }
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Security Profile", color = MaterialTheme.colorScheme.onSurface) },
        ...
        LazyColumn(
                ...
                Box(
                    ...
                    Column(
                        ...
                        ProfilePictureWithUpload(
                            ...
                            initials = uiState.user?.name?.take(2)?.uppercase() ?: "SG",
                            isUploading = uiState.uploadProgress,
                            onUploadClick = { imagePickerLauncher.launch("image/*") }
                        )

                        Text(
                            text = uiState.user?.name ?: "Security Guard",
                            fontSize = 24.sp,
            ...
            item {
                Column(
                    ...
                    ProfileField(
                        ...
                        label = "Assigned Gate",
                        value = editGateLocation,
                        isEditing = uiState.isEditMode,
                        onValueChange = { editGateLocation = it }
                    )
            ...
            // Preferences Card (Security-specific)
            item {
                Column(
                    modifier = Modifier
                        .fillMaxWidth()
                    ...
                    uiState.preferences?.let { prefs ->
                        ...
                        SettingSwitch(
                            icon = Icons.Filled.NotificationsActive,
                            label = "Entry Alerts",
                            description = "Notify for vehicle entries",
                        ...
                        SettingSwitch(
                            icon = Icons.Filled.NotificationsActive,
                            label = "Exit Alerts",
                            description = "Notify for vehicle exits",
                        ...
                        SettingSwitch(
                            ...
                            label = "Scan Success Sound",
                            description = "Play sound on successful scan",
                            isChecked = prefs.scanSuccessSound,
                            onCheckedChange = {
                                viewModel.updatePreferences(prefs.copy(scanSuccessSound = it))
                            }
                        )
                        ...
                        SettingSwitch(
                            ...
                            description = "Device vibration on actions",
                            isChecked = prefs.vibrationFeedback,
                            onCheckedChange = {
                                viewModel.updatePreferences(prefs.copy(vibrationFeedback = it))
                            }
                        )
                    }
                }
